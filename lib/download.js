...
class Downloader {
    ...
    async fetchArticles() {
        ...
        for (let i = 0; i < tocs.length; i++) {
            ...
            if (perToc.parent_uuid && perToc.parent_uuid.length > 0 && uuidToPath.get(perToc.parent_uuid)) {
                parentName = uuidToPath.get(perToc.parent_uuid);
            }
            let path = '';
            if (parentName.length > 0) {
                path = `${parentName}/${perToc.title}`;
            } else {
                path = filenamify(perToc.title);
            }
        ...
        for (let i = 0; i < realDocs.length; i++) {
            ...
            if (parentName.length > 0) {
                file._fullName = `${parentName}/${filenamify(file.title)}.md`
            } else {
    ...
    async clearGithubFile() {
        ...
        allPaths.add(this.repo_name);

        await this.dfsDelGithubFile(allPaths, this.repo_name)
    }
    // 递归检查删除语雀没有的文件
    async dfsDelGithubFile(allPaths, path, isMd) {
        if (this.repo_name !== path && !allPaths.has(path.substring(this.repo_name.length + 1, path.length)) ) {
            if (isMd) {
                await this.gh.delFile(path + ".md");
            } else {
                await this.gh.delFile(path);
            }
            return;
        }
        if (isMd) {
        ...
        const { data } = await this.gh.getFileSha(path);
        if (data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                let isMd = data[i].path.indexOf('.md') != -1;
                // 去除md
                let nextPath = isMd ? data[i].path.substring(0, data[i].path.length - 3) :
                    data[i].path;
                await this.dfsDelGithubFile(allPaths, nextPath, isMd);
            }
        }
    }
}
