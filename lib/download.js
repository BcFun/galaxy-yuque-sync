...
class Downloader {
    ...
    async fetchArticles() {
        ...
        for (let i = 0; i < realDocs.length; i++) {
            file = realDocs[i];
            let parentName = '';
            if (file.parent_uuid && file.parent_uuid.length > 0 && parentMap.get(file.parent_uuid)) {
                parentName = parentMap.get(file.parent_uuid);
            }
            ...
            parentMap.set(file.uuid, file._fullName);
